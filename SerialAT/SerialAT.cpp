...
/*
* Use something like Putty. Select COM port.
* To Send SMSs
Set the modem to text mode with the command: AT+CMGF=1.
Enter the recipient’s number: AT+CMGS="<recipient_number>".
After the AT+CMGS command, type your message.
After typing your message, send it by pressing Ctrl+Z (which terminates the message with the null char)

* To Read SMSs
This code sends the AT command AT+CMGF=1 to set the modem to text mode, then sends the AT command AT+CMGL="ALL" to list all SMS messages. AT+CMGL=? to see all possible values.
*/
...
/*
* Usage:
SerialAT.exe COM4: test
SerialAT.exe COM4 <international phone no> "Message":
*/
